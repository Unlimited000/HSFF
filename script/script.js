$(document).ready(function(){
    ...
    // #direct_cont 안에 있는 a를 누르면
        ...
        // 방금 눌린 그것(a중에 하나)이 가지고 있는 속성중에 href라는 속성의 값이 무엇인지 알아낸다.
        // 방금 눌린 그것(a중에 하나)안에 들어있는 글씨를 알아낸다.
        // #direct_title>span에 그 글씨를 집어넣는다.
    // #direct_go를 누르면
        // 만약 아까 알아뒀던 눌린 a의 href 속성 값이 공백이 아니라면
            // 아까 알아뒀던 눌린 a의 href 속성 값으로 이동!
    var url;
    $("#direct_cont>a").click(function(e){
        e.preventDefault();
        url = $(this).attr("href");
        var txt = $(this).text();
        $("#direct_title>span").text(txt);
    });
    var add = 0;
    ...
    // .tabbtn을 눌렀을 때
        // 방금눌린그것이 몇 번째인가 알아내서 th라 부르자.
        // 모든 .tabcont를 모조리 숨겨줌
        // 이번에 보여줘야할 .tabcont(탭콘트중에 th번째)를 보여준다.
    ...
    // 각각의 list-db에게 다음과 같이 하겠다.
        // li가 몇개가 있는지 개수를 파악.
        // 그 개수만큼 반복
            // 각 li안 처음 부분에 <span>태그를 만든다.
            // i가 9이하면 앞에 "0"을 붙이고
            // i가 9초과면 아무일도 안하고
            // 그 span 태그의 내용으로 그 숫자를 넣는다.
    ...
    // 리스트로 만든 표 안쪽에 셀들 높이 중 가장 큰 녀석으로 통일하기
    // 각각 .listtable>li 안에 있는 span 들이 몇개인가 개수를 알아냄. => num
    // 그 개수만큼 반복
    ...
    // section table 들에게 [각각] 다음과 같이 이야기하겠다.
        // 그녀석의 바로 윗부분에 "<div class='tablebox'>" 추가
...
//    }
    ...
    // 문서에서 select태그를 찾아서 각각 다음과 같이 이야기하겠다.
        ...
        // 그 .baji-mark의 내용을 넣어준다.
        // 원본 그것(select)을 숨겨준다.
        // 챔피언 = 0;
        // 첫번째 도전자 vs 챔피언(숫자가 더 큰 사람이 이김)
        // 두번째 도전자 vs 챔피언(첫번째 도전자), 이후 결과는 알 수 없음
        // 세번째 도전자 vs 챔피언(1?2?)
    ...
    $("select").each(function(){
        ...
        for(i=0; i<sellen; i++){
            ...
            chall[i] = Math.ceil(chall[i]) + 25; //chevron 공간 확보
        }
        for(i=0; i<sellen; i++){
            if(chall[i] > champ){
                champ = chall[i];
    ...
    // .baji-opt>span을 눌렀을 때
        ...
        // 방금누른그것이 span중에서 몇번째 였던가? => th
        // 방금누른그것의 부모의 부모의 형(select)의 자식중에 option중에서 th번째를 강제로 선택한것처럼 만든다.
